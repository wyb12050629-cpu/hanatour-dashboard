 ...
 * - 주차 기준일 = 해당 날짜가 속한 주의 다음 수요일
 * - 예: 4/1(수)~4/7(화) → 주차 기준일: 4/8(수)
 *
 * @returns {{ start: import('dayjs').Dayjs, end: import('dayjs').Dayjs, wednesday: string }}
 *   start: 이번 주차 시작일(수요일) / end: 화요일 23:59:59
 *   wednesday: 주차 기준일(다음 수요일) 'YYYY-MM-DD'
 */
export function getCurrentWeekRange() {
  ...
  const startWed = today.subtract(diffToWed, 'day').startOf('day');

  return {
    start: startWed,
    end: startWed.add(6, 'day').endOf('day'), // 화요일 23:59:59
    wednesday: startWed.add(7, 'day').format('YYYY-MM-DD'), // 주차 기준일 = 다음 수요일
  };
}
...
export function generateWeekOptions(year) {
  ...
  while (wed.isBefore(endOfYear) || wed.isSame(endOfYear, 'day')) {
    ...
    options.push({
      label,
      value: start.add(7, 'day').format('YYYY-MM-DD'), // 주차 기준일 = 다음 수요일
      start,
      end: end.endOf('day'),
    });
...
/**
 * 주어진 날짜의 주차 기준일(다음 수요일)을 반환합니다.
 *
 * 주차 기준일 = 해당 날짜가 속한 주(수~화)의 다음 수요일
 * 예: 4/1(수)~4/7(화) → 4/8(수), 4/8(수)~4/14(화) → 4/15(수)
 *
 * @param {string | import('dayjs').Dayjs | Date} date
 * @returns {string} 'YYYY-MM-DD' 형식의 다음 수요일 날짜
 */
export function getWeekWednesday(date) {
  const d = dayjs(date);
  const dow = d.day(); // 0=일, 1=월, 2=화, 3=수, 4=목, 5=금, 6=토

  let diff;
  if (dow === 3) diff = 7;       // 수요일: 다음 수요일(7일 후)
  else if (dow === 4) diff = 6;  // 목요일: 6일 후
  else if (dow === 5) diff = 5;  // 금요일: 5일 후
  else if (dow === 6) diff = 4;  // 토요일: 4일 후
  else if (dow === 0) diff = 3;  // 일요일: 3일 후
  else if (dow === 1) diff = 2;  // 월요일: 2일 후
  else if (dow === 2) diff = 1;  // 화요일: 1일 후
...
// ─────────────────────────────────────────────
// 헤더 매핑 (대리점관리_HI327 ↔ 김기웅 선임 거래처 양식)
// ─────────────────────────────────────────────

/**
 * 대리점 시트 헤더 후보 목록 — 첫 번째로 값이 채워진 컬럼을 채택합니다.
 * 김기웅 선임님 양식(거래처*)과 기존 양식(대리점*) 모두 지원합니다.
 */
const AGENCY_HEADER_MAP = {
  대리점코드: ['대리점코드', '거래처코드', '코드', 'code'],
  대리점명:   ['대리점명', '거래처명', 'agency'],
  대리점유형: ['대리점유형', '거래처유형', '구분', '타입', 'type'],
  대표자명:   ['대표자명', '대표자', '대표'],
  점형태:     ['점형태'],
  내부등급:   ['내부등급'],
  팀:         ['팀', 'team'],
...
/** 대리점 시트로 인식할 시트명(소문자/원문 모두 비교) */
const AGENCY_SHEET_CANDIDATES = [
  'Agency_Overview', 'agency_overview',
  'Sheet1', 'sheet1',
  '대리점', '대리점관리', 'agencies', '거래처', '거래처관리',
];

/**
 * 헤더 후보 배열에서 row의 첫 번째 매칭 값을 반환합니다.
 * @param {Object} row
...
/**
 * 워크북에서 대리점 시트를 자동 감지하여 반환합니다.
 * @param {Object} wb XLSX 워크북
 * @returns {Object | null}
 */
function findAgencySheet(wb) {
  for (const name of AGENCY_SHEET_CANDIDATES) {
    if (wb.Sheets[name]) return wb.Sheets[name];
  }
  // 마지막으로 워크북의 모든 시트를 대소문자 무시 매칭
  const lower = AGENCY_SHEET_CANDIDATES.map((s) => s.toLowerCase());
  for (const name of wb.SheetNames) {
    if (lower.includes(name.toLowerCase())) return wb.Sheets[name];
  }
  return null;
}
...
export async function parseExcelFile(file) {
  ...
  // ── 대리점 시트 (Agency_Overview · Sheet1 · 거래처 등 자동 감지) ────
  const wsAgency = findAgencySheet(wb);
  if (wsAgency) {
    const pick = (row, key) => pickField(row, AGENCY_HEADER_MAP[key]);
    out.agencies = wsToRows(wsAgency)
      .map((r) => ({
        대리점코드:   str(pick(r, '대리점코드')),
        대리점명:     str(pick(r, '대리점명')),
        대리점유형:   str(pick(r, '대리점유형')),
        대표자명:     str(pick(r, '대표자명')),
        점형태:       str(pick(r, '점형태')),
        내부등급:     str(pick(r, '내부등급')),
        팀:           str(pick(r, '팀')),
        부서:         str(pick(r, '부서')),
        담당자:       str(pick(r, '담당자')),
        보고구분:     str(pick(r, '보고구분')),   // '공식인증' | '일반대리점'
        관리등급:     str(pick(r, '관리등급')),   // 'A' | 'B' | 'C'
        특징:         str(pick(r, '특징')),
        다음액션:     str(pick(r, '다음액션')),
        최근접촉일:   toDateStr(pick(r, '최근접촉일')),
        메모:         str(pick(r, '메모')),
      }))
      .filter((r) => r.대리점코드);
  }
  ...
  if (wsDaily) {
    out.dailyLogs = wsToRows(wsDaily)
      .map((r) => {
        const dateStr = toDateStr(r['날짜']);
        return {
          날짜:       dateStr,
          // 주차(수요일)를 날짜 기반으로 재계산 (다음 수요일 기준)
          주차:       dateStr ? getWeekWednesday(dateStr) : toDateStr(r['주차(수요일)']),
          대리점코드:  str(r['대리점코드']),
          대리점명:    str(r['대리점명']),
          보고구분:    str(r['보고구분']),
