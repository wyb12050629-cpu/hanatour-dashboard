...
const STORAGE_KEY = 'hanatour_v2';

/**
 * localStorage에서 앱 상태를 복원합니다.
 * SSR 환경(window 없음)이나 파싱 실패 시 null을 반환합니다.
 *
 * @returns {{
 *   agencies: Array<Object>,
 *   quotes: Array<Object>,
...
export function loadFromStorage() {
  try {
    ...
    console.error('[store] loadFromStorage 실패:', err);
    return null;
  }
}

/**
 * 앱 상태를 localStorage에 직렬화하여 저장합니다.
 * SSR 환경에서는 아무것도 하지 않습니다.
 *
 * @param {{
 *   agencies: Array<Object>,
 *   quotes: Array<Object>,
...
export function saveToStorage(state) {
  try {
    ...
    console.error('[store] saveToStorage 실패:', err);
  }
}
