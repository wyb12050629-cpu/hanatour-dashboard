...
export async function bulkSaveAll({ agencies, quotes, dailyLogs, incentives }) {
  ...
  try {
    const promises = [];
    if (agencies?.length)   promises.push(supabase.from('agencies').upsert(agencies.map(agencyToRow), { onConflict: 'code' }));
    if (quotes?.length)     promises.push(supabase.from('quotes').upsert(quotes.map(quoteToRow), { onConflict: 'quote_id' }));
    if (dailyLogs?.length)  promises.push(supabase.from('daily_logs').upsert(dailyLogs.map(logToRow), { onConflict: 'id' }));
    if (incentives?.length) promises.push(supabase.from('incentives').upsert(incentives.map(incentiveToRow), { onConflict: 'quote_id' }));
